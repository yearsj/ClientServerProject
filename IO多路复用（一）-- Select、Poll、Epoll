《UNIX网络变成卷1：套接字联网API》中提到五种IO模型，关于这五种IO模型可以参考博文[IO模型浅析-阻塞、非阻塞、IO复用、信号驱动、异步IO、同步IO](https://segmentfault.com/a/1190000016359495)，本篇主要介绍IO多路复用的使用和编程。
...
select函数中存在三个fd_set集合，分别代表三种事件，\_\_readfds表示读描述符集合，\_\_writefds表示读描述符集合，\_\_exceptfds表示读描述符集合，当对应的fd_set = NULL时，表示不监听该类描述符。

#### \_\_nfds

\_\_nfds是fd_set中最大的描述符+1，当调用select的时候，内核态会判断fd_set中描述符是否就绪，\_\_nfds告诉内核最多判断到哪一个描述符。
...
参数\_\_timeout指定select的工作方式：

- \_\_timeout= NULL，表示select永远等待下去，直到其中至少存在一个描述符就绪
- \_\_timeout结构体中秒或者微妙是一个大于0的整数，表示select等待一段固定的事件，若该短时间内未有描述符就绪则返回
- \_\_timeout= 0，表示不等待，直接返回
...
\_\_fds的作用同select中的\_\_nfds，表示pollfd数组中最大的下标索引

#### \_\_timeout

- \_\_timeout = -1：poll阻塞直到有事件产生
- \_\_timeout = -0：poll立刻返回
- \_\_timeout != -1 && \_\_timeout != 0：poll阻塞\_\_timeout对应的时候，如果超过该时间没有事件产生则返回
...
epoll_create函数创建一个epoll实例并返回，该实例可以用于监控\_\_size个文件描述符
...
该函数用来向epoll中注册事件函数，其中\_\_epfd为epoll_create返回的epoll实例，\_\_op表示要进行的操作，\_\_fd为要进行监控的文件描述符，\_\_event要监控的事件。

\_\_op可用的类型定义在sys/epoll.h头文件中，如下：
...
epoll_wait类似与select中的select函数、poll中的poll函数，等待内核返回监听描述符的事件产生，其中\_\_epfd是epoll_create创建的epoll实例，__events数组为epoll\_wait要返回的已经产生的事件集合，其中第i个元素成员的\_\_events[i]->data->fd表示产生该事件的描述符，\_\_maxevents为希望返回的最大的事件数量（通常为\_\_events的大小），\_\_timeout和poll中的\_\_timeout相同。该函数返回已经就绪的事件的数量，如果为-1表示出错。
...
可以使用一个生活中的场景来对三者的区别做个总结，仍然接着笔者的上一篇博文[IO模型浅析-阻塞、非阻塞、IO复用、信号驱动、异步IO、同步IO](https://segmentfault.com/a/1190000016359495)中吃饭的例子：
...
另外关于epoll的高效还有很多细节，例如使用mmap将用户空间和内核空间的地址映射到同一块物理内存地址，使用红黑树存储要监听的事件等等，具体的细节可以参考博文[select、poll、epoll之间的区别总结整理](https://www.cnblogs.com/Anker/p/3265058.html)、[高并发网络编程之epoll详解](https://blog.csdn.net/shenya1314/article/details/73691088)、[Linux下的I/O复用与epoll详解](https://www.cnblogs.com/lojunren/p/3856290.html)、[彻底学会使用epoll(一)——ET模式实现分析](http://blog.chinaunix.net/uid-28541347-id-4273856.html)等几篇文章。
